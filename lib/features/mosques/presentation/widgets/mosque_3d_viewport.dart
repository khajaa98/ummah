...
class _Mosque3DViewportState extends ConsumerState<Mosque3DViewport>
  ...
  void _applyTier(int tier) {
    ...
    // NOTE: flutter_3d_controller v2.x removed per-object visibility toggling.
    // Tier progression is now driven by a swap-in GLB strategy (TODO) or by
    // playing a named animation on the rigged model. For now the model
    // renders in full; the upgrade flash + counter still fire so the user
    // sees positive feedback when their tier increments.
    // See: https://pub.dev/packages/flutter_3d_controller/changelog
    debugPrint('[Mosque3D] Tier $tier reached (visual swap not yet wired).');
  }
        ...
        children: [
          ...
          RepaintBoundary(
            child: Flutter3DViewer(
              ...
              onProgress: (_) {},
              onError: (err) => debugPrint('[Mosque3D] Failed to load mosque.glb: $err'),
            ),
          ),
