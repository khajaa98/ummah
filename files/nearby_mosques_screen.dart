...
class _NearbyMosquesScreenState extends ConsumerState<NearbyMosquesScreen> {
  ...
  void initState() {
    super.initState();
    // Trigger the initial fetch after the first frame so the widget tree is
    // fully mounted before we kick off the GPS + HTTP pipeline.
      ...
      body: mosquesAsync.when(
        ...
        data: (mosques) => RefreshIndicator(
          ...
          child: mosques.isEmpty
              // Provider just initialised (build() returns []) — show nothing
              // until the PostFrameCallback triggers fetchNearby().
              // Once fetching starts, state transitions to AsyncLoading.
